...
// eCAL publisher
const eCAL::CPublisher *Scene::publisher_ = nullptr;
...
void Scene::createGeometryFrameBuffer() {
  ...
  for (GLuint i = 0; i < TEXTURE_BUFFERS; i++) { attachment[i] = GL_COLOR_ATTACHMENT0 + i; }
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) { std::cerr << "error: the geometry frame buffer object status is not complete (" << status << ")" << std::endl; }
...
// GLFW error callback
void Scene::errorCallback(int error, const char *description) { std::cerr << "error " << error << ": " << description << std::endl; }
...
void Scene::framebufferSizeCallback(GLFWwindow *window, int width, int height) {
  ...
  for (const std::pair<const std::size_t,Camera *const> &camera_data : scene->camera_stock) { camera_data.second->setResolution(resolution); }
}
...
void Scene::drawScene() {
  ...
  for (const std::pair<const std::size_t,std::pair<const Model *const,const std::size_t>> model_data : model_stock) {
    ...
    if (!model_data.second.first->isOpen()) { continue; }
    // Get the program
...
Scene::Scene(const std::string &title, const int &width, const int &height, const int &context_ver_maj, const int &context_ver_min)
  ...
  if (window == nullptr) { std::cerr << "error: cannot create the application window" << std::endl; }
...
// Get title
std::string Scene::getTitle() const { return title; }

// Get resolution
glm::vec2 Scene::getResolution() const { return glm::vec2(width, height); }

// Get the background color
glm::vec3 Scene::getBackgroundColor() const { return background_color; }


// Get the active camera
Camera *Scene::getActiveCamera() const { return active_camera; }
...
std::size_t Scene::getLightingPassProgramID() const { return lighting_program; }


// Get the default geometry pass program
GLSLProgram *Scene::getDefaultGeometryPassProgram() { return program_stock[0U].first; }

// Get the default geometry pass program description
std::string Scene::getDefaultGeometryPassProgramDescription() { return program_stock[0U].second; }


// Get the default lighing pass program
GLSLProgram *Scene::getDefaultLightingPassProgram() { return program_stock[1U].first; }

// Get the default lighing pass program description
std::string Scene::getDefaultLightingPassProgramDescription() { return program_stock[1U].second; }


// Get frames
double Scene::getFrames() const { return kframes; }


// Setters

// Set the background color
void Scene::setBackgroundColor(const glm::vec3 &color) { background_color = color; }
...
bool Scene::selectCamera(const std::size_t &id) {
  ...
  if (result == camera_stock.end()) { return false; }
...
bool Scene::setProgramDescription(const std::string &desc, const std::size_t &id) {
  ...
  if (result == program_stock.end()) { return false; }
...
void Scene::setLightingPassProgram(const std::size_t &id) { lighting_program = id; }
...
void Scene::setDefaultGeometryPassProgramDescription(const std::string &desc) { program_stock[0U].second = desc; }
...
void Scene::setDefaultLightingPassProgramDescription(const std::string &desc) { program_stock[1U].second = desc; }
...
// Reload all programs
void Scene::reloadPrograms() { for (std::pair<const std::size_t,std::pair<GLSLProgram *,std::string>> &program_data : program_stock) { program_data.second.first->link(); } }
...
bool Scene::removeCamera(const std::size_t &id) {
  ...
  if (result->second == active_camera) { active_camera = std::next(result, result->second == camera_stock.rbegin()->second ? -1 : 1)->second; }
...
bool Scene::removeModel(const std::size_t &id) {
  ...
  if (result == model_stock.end()) { return false; }
...
bool Scene::removeLight(const std::size_t &id) {
  ...
  if (result == light_stock.end()) { return false; }
...
Scene::~Scene() {
  // Delete cameras
  for (const std::pair<const std::size_t,const Camera *const> &camera_data : camera_stock) { delete camera_data.second; }

  // Delete models
  for (const std::pair<const std::size_t,std::pair<const Model *const,std::size_t>> &model_data : model_stock) { delete model_data.second.first; }

  // Delete programs
  for (const std::pair<const std::size_t,std::pair<const GLSLProgram *const,const std::string>> &program_data : program_stock) { delete program_data.second.first; }
...
const GLubyte *Scene::getOpenGLVendor() { return Scene::opengl_vendor; }

// Get the OpenGL renderer
const GLubyte *Scene::getOpenGLRenderer() { return Scene::opengl_renderer; }

// Get the OpenGL version
const GLubyte *Scene::getOpenGLVersion() { return Scene::opengl_version; }

// Get the GLSL version
const GLubyte *Scene::getGLSLVersion() { return Scene::glsl_version; }
