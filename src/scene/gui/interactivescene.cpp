...
void InteractiveScene::showMainGUIWindow() {
  ...
  //ImGui::SetNextWindowSize(ImVec2(1000.0F, static_cast<float>(height)), ImGuiCond_Always);
  ...
  const bool open = ImGui::Begin("Settings", &show_main_gui, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBringToFrontOnFocus);
  ImGui::PopStyleVar();
  ...
  if (ImGui::CollapsingHeader("Scene")) {
    ...
    if (ImGui::TreeNode("Statistics*")) {
      ...
      for (const std::pair<const std::size_t,std::pair<const GLSLProgram *const,const std::string>> &program_data : program_stock) { shaders += program_data.second.first->getNumberOfShaders(); }
  ...
  if (ImGui::CollapsingHeader("Cameras")) {
    ...
    if (ImGui::TreeNodeEx("Global", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::DragFloat("Sensibility", &value, 0.25F, 0.0F, 0.0F, "%.4f")) { Camera::setSensibility(value); }
      // Speed
      value = Camera::getSpeed();
      if (ImGui::DragFloat("Speed", &value, 0.005F, 0.0F, FLT_MAX, "%.4f")) { Camera::setSpeed(value); }
      // Boost speed
      value = Camera::getBoostedSpeed();
      if (ImGui::DragFloat("Boost speed", &value, 0.05F, 0.0F, FLT_MAX, "%.4f")) { Camera::setBoostedSpeed(value); }
    ...
    for (const std::pair<const std::size_t,Camera *const> &camera_data : camera_stock) {
      ...
      if (ImGui::TreeNode(id.c_str(), camera_title.c_str())) {
        if (!cameraWidget(camera_data.second, camera_data.first)) { remove = camera_data.first; }
        ImGui::TreePop();
      }
    }

    // Remove camera
    if (remove != 0U) { removeCamera(remove); }

    // Add camera button
    ImGui::Spacing();
    if (ImGui::Button("Add camera", ImVec2(454.0F, 19.0F))) { addCamera(); }
    ImGui::Spacing();
  }
  ...
  if (ImGui::CollapsingHeader("Models")) {
    ...
    // Remove model
    if (remove != 0U) { removeModel(remove); }

    // Add model button
    ImGui::Spacing();
    if (ImGui::Button("Add model", ImVec2(454.0F, 100.0F))) { addModel(); }
    ImGui::Spacing();
  }
  ...
  if (ImGui::CollapsingHeader("Lights")) {
    ...
    for (const std::pair<const std::size_t,Light *const> &light_data : light_stock) {
      ...
      if (ImGui::TreeNode(id.c_str(), light_title.c_str())) {
        if (!lightWidget(light_data.second)) { remove = light_data.first; }
        ImGui::TreePop();
      }
    }

    // Remove light
    if (remove != 0U) { removeLight(remove); }

    // Add light button
    ImGui::Spacing();
    if (ImGui::Button("Add light", ImVec2(454.0F, 19.0F))) { addLight(); }
    ImGui::Spacing();
  }
  ...
  if (ImGui::CollapsingHeader("GLSL Programs")) {
    ...
    if ((lighting_program != 0U) && (lighting_program != 1U)) { program_title.append(" (").append(std::to_string(lighting_program)).append(")"); }
    ...
    if (ImGui::BeginCombo("###lighting_pass_program", program_title.c_str())) {
      ...
      for (const std::pair<const std::size_t,std::pair<const GLSLProgram *const,const std::string>> &program_data : program_stock) { if (programComboItem(lighting_program, program_data.first)) { new_program = program_data.first; } }
    ...
    for (std::pair<const std::size_t,std::pair<GLSLProgram *,std::string>> &program_data : program_stock) {
      ...
      switch (program_data.first) {
        ...
        case 0U: program_title = "Default geometry pass";
          break;

        // Default lighting pass program
        case 1U: program_title = "Default lighting pass";
          break;

        // Other programs
        default: program_title = "Program " + id + ": " + program_data.second.second;
      }

      // Draw node and catch the selected to remove
      if (ImGui::TreeNode(id.c_str(), program_title.c_str())) {
        if (!programWidget(program_data.second)) { remove = program_data.first; }
        ImGui::TreePop();
      }
    }

    // Remove program
    if (remove != 0U) { removeProgram(remove); }

    // Add program button
    ImGui::Spacing();
    if (ImGui::Button("Add GLSL program", ImVec2(454.0F, 19.0F))) { addProgram("Empty"); }
    ImGui::Spacing();
  }
...
bool InteractiveScene::modelWidget(std::pair<Model *,std::size_t> &model_data) {
  ...
  if (ImGui::InputText("Path", &str, ImGuiInputTextFlags_EnterReturnsTrue)) { model->setPath(str); }
  // Enabled status
  bool enabled = model->isEnabled() && model->isOpen();
  if (ImGui::Checkbox("Enabled", &enabled)) { model->setEnabled(enabled && model->isOpen()); }
  // Reload button
  ImGui::SameLine();
  if (ImGui::Button("Reload model")) { model->reload(); }
  // Remove button
  keep = !ImGui::RemoveButton();

  // Check open status
  if (!model->isOpen()) {
    if (!model->Model::getPath().empty()) { ImGui::TextColored(ImVec4(0.80F, 0.16F, 0.16F, 1.00F), "Could not open the model"); }
    return keep;
  }
  ...
  if (ImGui::BeginCombo("GLSL program", program_title.c_str())) {
    ...
    for (const std::pair<const std::size_t,std::pair<const GLSLProgram *const,const std::string>> &program_data : program_stock) { if (programComboItem(model_data.second, program_data.first)) { new_program = program_data.first; } }
  ...
  if (ImGui::TreeNodeEx("Geometry", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::DragFloat3("Position", &value.x, 0.01F, 0.0F, 0.0F, "%.4f")) { model->Model::setPosition(value); }

    // Rotation
    value = model->Model::getRotationAngles();
    if (ImGui::DragFloat3("Rotation", &value.x, 0.50F, 0.0F, 0.0F, "%.4f")) { model->Model::setRotation(value); }
    ImGui::HelpMarker("Angles in degrees");

    // Scale
    value = model->Model::getScale();
    if (ImGui::DragFloat3("Scale", &value.x, 0.01F, 0.0F, 0.0F, "%.4f")) { model->setScale(value); }
  ...
  if (ImGui::TreeNode("Materials")) {
    ...
    // Material material
    if (!str.empty()) {
      if (ImGui::Button("Reload material")) { model->reloadMaterial(); }
      ImGui::HelpMarker("If the material data has\nchanged, nothing happens");
    }

    // Check material status
    if (!model->isMaterialOpen()) { ImGui::TextColored(ImVec4(0.80F, 0.16F, 0.16F, 1.00F), "Could not open the material file"); }
    ...
    if (ImGui::TreeNodeEx("Global", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::ColorEdit3("Ambient", &color.x)) {
        ...
        for (std::size_t i = 0U; i < materials; i++) { model->getMaterial(i)->setColor(material_attribute, color); }
      }
      ...
      if (ImGui::DragFloat("Shininess", &value, 0.01F, 0.0F, 1000.0F, "%.4f")) {
        ...
        for (std::size_t i = 0U; i < materials; i++) { model->getMaterial(i)->setValue(material_attribute, value); }
      }
      ...
      if (ImGui::TreeNodeEx("Textures", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::Checkbox("Ambient", &enabled)) {
          ...
          for (std::size_t i = 0U; i < materials; i++) { model->getMaterial(i)->setTextureEnabled(material_attribute, enabled); }
        }
    ...
    for (std::size_t i = 0; i < materials; i++) {
      ...
      if (ImGui::TreeNode(id.c_str(), name.c_str())) {
        // Material name
        if (ImGui::InputText("Name", &name, ImGuiInputTextFlags_EnterReturnsTrue)) { material->setName(name); }

        // Ambient color
        material_attribute = Material::AMBIENT;
        color = material->getColor(material_attribute);
        if (ImGui::ColorEdit3("Ambient", &color.x)) { material->setColor(material_attribute, color); }

        // Diffuse color
        material_attribute = Material::DIFFUSE;
        color = material->getColor(material_attribute);
        if (ImGui::ColorEdit3("Diffuse", &color.x)) { material->setColor(material_attribute, color); }

        // Specular color
        material_attribute = Material::SPECULAR;
        color = material->getColor(material_attribute);
        if (ImGui::ColorEdit3("Specular", &color.x)) { material->setColor(material_attribute, color); }

        // Shininess value
        material_attribute = Material::SHININESS;
        value = material->getValue(material_attribute);
        if (ImGui::DragFloat("Shininess", &value, 0.01F, 0.0F, 1000.0F, "%.4f")) { material->setValue(material_attribute, value); }

        // Roughness value
        material_attribute = Material::ROUGHNESS;
        value = material->getValue(material_attribute);
        if (ImGui::DragFloat("Roughness", &value, 0.005F, 0.0F, 1.0F, "%.4f")) { material->setValue(material_attribute, value); }

        // Metalness value
        material_attribute = Material::METALNESS;
        value = material->getValue(material_attribute);
        if (ImGui::DragFloat("Metalness", &value, 0.005F, 0.0F, 1.0F, "%.4f")) { material->setValue(material_attribute, value); }

        // Parallax value
        material_attribute = Material::DISPLACEMENT;
        value = material->getValue(material_attribute);
        if (ImGui::DragFloat("Parallax", &value, 0.005F, 0.0F, 0.0F, "%.4f")) { material->setValue(material_attribute, value); }
        ...
        if (ImGui::TreeNode("Textures")) {
          ...
          for (const std::pair<const Material::Attribute,const std::string> &texture : InteractiveScene::AVAILABLE_TEXTURE) {
            ...
            if (ImGui::TreeNode(id.c_str(), texture.second.c_str())) {
              ...
              if (ImGui::InputText("Path", &str, ImGuiInputTextFlags_EnterReturnsTrue)) { material->setTexturePath(texture.first, str); }

              // Enabled status
              enabled = material->isTextureEnabled(texture.first);
              if (ImGui::Checkbox("Enabled", &enabled)) { material->setTextureEnabled(texture.first, enabled); }

              // Reload buttons
              ImGui::SameLine();
              if (ImGui::Button("Reload texture")) { material->reloadTexture(texture.first); }
...
InteractiveScene::InteractiveScene(const std::string &title, const int &width, const int &height, const int &context_ver_maj, const int &context_ver_min)
  ...
  if ((Scene::instances == 1U) && Scene::initialized_glad) {
    ...
    ImGuiIO &io = ImGui::GetIO();
    io.Fonts->AddFontFromFileTTF("JetBrainsMono-Regular.ttf", 50, nullptr, io.Fonts->GetGlyphRangesChineseFull());
